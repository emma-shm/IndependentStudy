...
class SatelliteControlEnv(gym.Env):         #creating class that has the basic structure and functionality of the parent class, gym.Env, which is just how environment classes are defined for SB3
    ...
    def __init__(self, target_altitude=400000, max_thrust=10, max_mass=1000):
        ...
        self.H = 51600  # Scale height (m) for atmospheric model
        ...
        self.state = np.array([self.target_altitude + 10000, 7800, self.max_mass])  # Initial state placeholder
    ...
    def step(self, action):
        ...
        print(
            f"New Radius: r_new={r_new:.2f}m, Altitude_new={altitude_new:.2f}m, Velocity Magnitude_new={v_magnitude_new:.2f}m/s")
        ...
        reward = -1 * altitude_error - 0.1 * velocity_error - 0.1 * thrust_magnitude
        print(
            f"Reward Components: altitude_error={altitude_error:.2f}m, v_ideal={v_ideal:.2f}m/s, velocity_error={velocity_error:.2f}m/s")
